...
export function resolveBrand(brand: TenantBrand) {
  ...
  return {
    ...
    logoHeight: (brand.logoHeight as number | undefined) ?? 32,
    ...
    // heroCta1Color now defaults to buttonColor so "Button" controls all CTAs
    heroCta1Color: str(brand.heroCta1Color, str(brand.buttonColor as string | undefined, primary)),
    ...
    // Trust badges: if key is present in DB but empty → hide (return ""); if absent → use default
    trustBadge1: "trustBadge1" in brand ? (brand.trustBadge1 as string ?? "") : GENERIC_DEFAULTS.trustBadge1,
    trustBadge2: "trustBadge2" in brand ? (brand.trustBadge2 as string ?? "") : GENERIC_DEFAULTS.trustBadge2,
    trustBadge3: "trustBadge3" in brand ? (brand.trustBadge3 as string ?? "") : GENERIC_DEFAULTS.trustBadge3,
    trustBadge4: "trustBadge4" in brand ? (brand.trustBadge4 as string ?? "") : GENERIC_DEFAULTS.trustBadge4,
