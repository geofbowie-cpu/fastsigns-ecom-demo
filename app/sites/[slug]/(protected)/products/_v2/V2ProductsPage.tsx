// V2 Products listing — Amazon-style utility-first layout.
...
// ─── sub-components (all inline, server-only unless noted) ──────────────────
  ...
  return (
    ...
    >
      <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 py-3 flex items-center gap-4">
        ...
        {/* Search box — client component for URL-driven filtering */}
        <Suspense
          fallback={
            <div className="flex-1 max-w-2xl rounded-card border-2 border-white/20 bg-white/10 h-10" />
          }
        >
          <V2SearchBox
            slug={slug}
            defaultValue={q}
            navTextColor={b.navTextColor}
          />
        </Suspense>
...
function FilterSidebar({
  ...
  const leadTimes = unique(all.map((p) => p.leadTime).filter(Boolean))
  const materials = unique(
    all.flatMap((p) => (p.materials.length > 0 ? [p.materials[0]] : [])).filter(Boolean)
  ).slice(0, 6)

  const chevron = (
    <svg
      className="h-4 w-4 text-ink-400 group-open:rotate-180 transition-transform"
      viewBox="0 0 20 20"
      fill="currentColor"
      aria-hidden
    >
  ...
  return (
    <div className="flex-1 min-w-0">
      ...
      <div className="flex items-center justify-between mb-4">
        <div>
          {q && (
            <p className="text-xs text-ink-600 mb-1">
              Results for &ldquo;<span className="font-semibold text-ink-900">{q}</span>&rdquo;
            </p>
          )}
          {activeCategory && (
            <h1 className="font-display font-bold text-ink-900 text-xl mb-1">
              {activeCategory.name}
            </h1>
          )}
        ...
        {/* Sort by mock — URL-driven, no client state */}
        <select
          className="text-xs border border-ink-200 rounded-card px-2 py-1.5 text-ink-700 bg-white cursor-pointer"
      ...
      {filtered.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-24 text-center bg-white rounded-card border border-ink-200 shadow-soft">
          <svg
            className="h-10 w-10 text-ink-400 mb-3"
            viewBox="0 0 24 24"
            fill="none"
            stroke="currentColor"
            strokeWidth={1.5}
          ...
          <p className="text-xs text-ink-600 max-w-xs">
            {q
              ? `No products match "${q}". Try a different search term or browse all categories.`
              : "No products available in this category yet. Check back soon."}
          </p>
          <Link
            href={`/sites/${slug}/products`}
            className="mt-4 text-xs font-semibold text-ink-600 hover:text-ink-900 underline"
          >
            Browse all products
          </Link>
        </div>
...
export default function V2ProductsPage({
  ...
  return (
    <div className={cn(v2FontVars, "font-body bg-ink-50 min-h-screen flex flex-col")}>
      ...
      {/* 3. Main header (with live search box) */}
      <MainHeader b={b} slug={slug} q={q} />

      {/* 4. Departments strip (highlights active category) */}
      <DepartmentsStrip
        cats={cats}
        slug={slug}
