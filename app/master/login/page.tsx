...
function MasterLogin() {
  ...
  async function signInWithPassword(e: React.FormEvent) {
    ...
    setBusy(true)
    try {
      const res = await fetch("/api/auth/master-password-login", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
    ...
    <div className="min-h-screen bg-gray-900 flex items-center justify-center p-6">
      <div className="w-full max-w-md">
        <div className="bg-white rounded-2xl shadow-2xl p-8">
          <div className="mb-6">
            <div className="text-xs font-bold uppercase tracking-widest text-gray-500 mb-1">FASTSIGNS</div>
          ...
          {mode === "link" && (
            sent ? (
              ...
              <>
                ...
                <form onSubmit={(e) => { e.preventDefault(); sendLink(email) }}>
          ...
          {mode === "password" && (
            <form onSubmit={signInWithPassword}>
              ...
              <p className="mt-4 text-xs text-center text-gray-400">
                No password yet? Ask your admin to set one, or use the Email link tab.
              </p>
            </form>
          )}
        </div>
        <p className="text-center text-xs text-gray-500 mt-6">Need access? Contact your administrator.</p>
      </div>
    </div>
  )
}
